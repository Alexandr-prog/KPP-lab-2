    ...
    for ( let y = 1; y < currentBoard.length-1; y++) {
      ...
      if (currentBoard[x][y] === '#') {
        if (total <= 1 || total >= 4) {
          ...
          GameBoard[x][y] = '#'; //Any live cell with two or three live neighbours lives on to the next generation.
        }
      }

      if (total === 3) {
        game[x][y] = '#'; //Any dead cell with exactly three live neighbours becomes a live cell, as if by reproduction.
      }
    }
  }
}
